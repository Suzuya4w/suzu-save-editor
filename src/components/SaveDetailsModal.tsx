...
import { BadgeCheck, FileCheck2, ShieldCheck, Loader2, ShieldAlert, Flag, Clock, HardDrive, Download, Gamepad2 } from 'lucide-solid';
     ...
     <div class="-m-6 flex flex-col h-full bg-[#0a0a0a]">
      ...
      <div class="flex-1 overflow-y-auto p-0 flex flex-col md:flex-row bg-[#0a0a0a]">
        <div class="w-full md:w-2/5 border-r border-zinc-800 bg-black flex flex-col overflow-hidden">
         <Show when={props.save?.game_cover_url}>
          <div class="w-full h-64 md:h-full relative shrink-0">
           <img src={props.save?.game_cover_url} class={`w-full h-full object-cover ${props.save?.is_nsfw && !props.showNSFW ? 'blur-xl scale-110' : ''}`} />
           <Show when={props.save?.is_nsfw && !props.showNSFW}>
         ...
         <Show when={!props.save?.game_cover_url}>
          <div class="w-full h-64 md:h-full bg-zinc-900 flex items-center justify-center">
        ...
        <div class="w-full md:w-3/5 p-6 flex flex-col">
         ...
         <div class="flex items-center justify-between mb-6 pb-4 border-b border-zinc-800">
           <div class="flex items-center gap-3 bg-zinc-900 p-2 pr-4 border border-zinc-700 rounded-sm w-fit">
            <Show when={props.save?.uploader_avatar_url}>
             <img src={props.save?.uploader_avatar_url} class="w-26 h-26 object-cover border border-zinc-600 rounded-full" />
            </Show>
            <div class="flex flex-col">
             ...
             <span class="text-xs font-black text-white uppercase tracking-widest flex items-center gap-2 leading-none mt-1">
              {props.save?.uploader}
              <Show when={props.save?.is_verified}>
           ...
           <div class="flex flex-col items-end text-right">
            <span class="text-[10px] text-zinc-500 uppercase font-bold tracking-widest">Extension</span>
            <span class="text-sm font-black text-white tracking-widest uppercase flex items-center gap-1"><FileCheck2 size={14}/> {props.save?.extension || '.sav'}</span>
           </div>
         </div>
         ...
         <div class="flex-1 mb-6">
          ...
          <p class="text-sm text-zinc-300 font-desc whitespace-pre-wrap">{props.save?.description || 'No description provided.'}</p>
         </div>
         
         <div class="flex items-center gap-2 flex-wrap mb-6">
          <Show when={props.save?.scan_status === 'safe'}>
          ...
          <Show when={props.save?.scan_status === 'scanning' || props.save?.scan_status === 'pending'}>
          ...
          <Show when={props.save?.scan_status === 'malicious'}>
          ...
          <Show when={props.isAdminMode}>
           <div class={`px-3 py-1.5 border flex items-center gap-2 rounded-sm text-xs font-black uppercase tracking-widest ${(props.save?.report_count || 0) > 0 ? 'bg-red-950/30 border-red-900 text-red-500' : 'bg-green-950/30 border-green-900 text-green-500'}`}>
            {(props.save?.report_count || 0) > 0 ? <Flag size={14}/> : <ShieldCheck size={14}/>}
            {(props.save?.report_count || 0)} Reports
           </div>
          </Show>
         </div>
         ...
         <div class="flex items-center justify-between text-xs text-zinc-500 border-t border-zinc-800 pt-4 font-bold tracking-widest uppercase">
           <div class="flex items-center gap-2">
            <Clock size={14} />
            {new Date(props.save?.created_at).toLocaleString([], { year: 'numeric', month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
           </div>
           <div class="flex items-center gap-2">
            <HardDrive size={14} />
            {formatBytes(props.save?.file_size_bytes || 0)}
           </div>
         </div>
        </div>
       </div>
       ...
       <div class="p-4 bg-black border-t-4 border-zinc-800 flex justify-between gap-4">
        <button onClick={() => { props.onReport(props.save!.id, props.save!.title); }} class="px-4 py-3 bg-zinc-950 text-zinc-500 border-2 border-zinc-800 hover:text-white hover:border-red-500 transition-colors flex items-center gap-2 uppercase font-black tracking-widest text-xs cursor-pointer rounded-sm">
         <Flag size={14} /> REPORT
        </button>
        <button onClick={() => { props.onDownload(props.save!.file_url, props.save!.title); }} class="px-8 py-3 bg-[#FF7A00] text-black border-2 border-black hover:bg-white hover:border-black transition-colors uppercase font-black tracking-widest text-xs shadow-[4px_4px_0px_#ffffff] flex items-center gap-2 cursor-pointer">
