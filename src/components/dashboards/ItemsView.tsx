...
import { Package, Plus, Trash2, Check, X } from 'lucide-solid';
...
export const ItemsView = () => {
  ...
  const [manualItemId, setManualItemId] = createSignal("");
  const [selectedItemIds, setSelectedItemIds] = createSignal<string[]>([]);
  const [newItemQty, setNewItemQty] = createSignal(1);
  const [itemToDelete, setItemToDelete] = createSignal<{id: string, name?: string} | null>(null);
  ...
  const filteredDatabase = createMemo(() => {
    const db = store.gameDatabase?.items;
    if (!db) return [];
  ...
  const hasDatabase = createMemo(() => {
    return store.gameDatabase?.items && Object.keys(store.gameDatabase.items).length > 0;
  });

  const itemsList = createMemo(() => {
    const rawItems = store.saveData?.parsed_variables?.party?._items || {};
  ...
  const virtualizer = createVirtualizer({
    get count() { return itemsList().length; },
  ...
  const handleQuantityChange = (itemId: string, newQuantity: string) => {
    ...
    if (!isNaN(val)) {
      updateValue(`party._items.${itemId}`, val);
    }
  };
  ...
  const handleConfirmAdd = () => {
    ...
    try {
      ...
      if (hasDatabase()) {
        ...
        ids.forEach(id => { updateValue(`party._items.${id}`, qty); addedCount++; });
      } else {
        const ids = manualItemId().split(',').map(s => s.trim()).filter(s => s && !isNaN(Number(s)));
        ids.forEach(id => { updateValue(`party._items.${id}`, qty); addedCount++; });
      }

      if (addedCount > 0) {
        addToast(`Successfully injected ${addedCount} item(s)`, 'success');
      } else {
      ...
      addToast('Failed to inject item(s)', 'error');
    }
  };
  ...
  const confirmDelete = () => {
    ...
    if (item) {
      deleteValue(`party._items.${item.id}`);
      setSelectedForBulkDelete(prev => prev.filter(id => id !== item.id));
      addToast(`Deleted item ID ${item.id} successfully`, 'success');
  ...
  const confirmBulkDelete = () => {
    ...
    ids.forEach(id => deleteValue(`party._items.${id}`));
    addToast(`Deleted ${ids.length} item(s) successfully`, 'success');
    ...
    <div class="flex flex-col gap-8 w-full max-w-5xl mx-auto relative h-full flex-1 min-h-0">
      ...
      <div class="flex items-center justify-between border-b border-zinc-800 pb-4 shrink-0">
        ...
        <div class="flex items-center gap-2">
          ...
          <Show when={!isSelectionMode() && itemsList().length > 0}>
            <button 
        ...
        <div class={`grid ${isSelectionMode() ? 'grid-cols-[60px_80px_100px_1fr_120px]' : 'grid-cols-[80px_100px_1fr_120px]'} gap-4 items-center p-4 border-b-2 border-zinc-800 bg-zinc-950 text-zinc-500 text-[10px] font-black uppercase tracking-widest shrink-0`}>
          <Show when={isSelectionMode()}>
            <div class="flex items-center justify-center">
              <div 
                onClick={() => {
                  if (itemsList().length > 0 && selectedForBulkDelete().length === itemsList().length) {
                    setSelectedForBulkDelete([]);
                  } else {
                    setSelectedForBulkDelete(itemsList().map(i => i.id));
                  }
                }}
                class={`w-16 h-16 rounded-full border-2 flex items-center justify-center cursor-pointer transition-all ${
                  itemsList().length > 0 && selectedForBulkDelete().length === itemsList().length
                    ? 'bg-[#FF5252] border-[#FF5252] opacity-100'
                    : 'border-zinc-400 opacity-50 hover:opacity-100'
                }`}
              >
                <Show when={itemsList().length > 0 && selectedForBulkDelete().length === itemsList().length}>
                    ...
                    >
                      ...
                      <div class="flex flex-col overflow-hidden">
                        <span class="text-zinc-200 font-bold uppercase tracking-widest text-sm truncate">
                          {store.gameDatabase?.items?.[Number(item.id)] || `Item #${item.id}`}
                        </span>
                        <Show when={store.gameDatabase?.items?.[Number(item.id)]}>
                          <span class="text-[10px] text-[#00F0FF] font-fira tracking-widest uppercase mt-0.5">
                            ORIGINAL ID: {item.id}
                          </span>
                        </Show>
                      </div>
                      ...
                      <div class="flex justify-end items-center gap-3 shrink-0">
                        <input 
                          type="number" 
                          value={item.quantity}
                          onInput={(e) => handleQuantityChange(item.id, e.target.value)}
                        ...
                        <button
                          onClick={() => setItemToDelete({
                            id: item.id,
                            name: store.gameDatabase?.items?.[Number(item.id)] || `Item #${item.id}`
                          })}
      ...
      <Modal
        ...
        title="INJECT NEW ITEM"
        icon={<Package size={18} class="text-[#00F0FF]" />}
        width="max-w-sm"
      >
        <div class="flex flex-col gap-6 font-brains">
          <div class="flex flex-col gap-4">
            ...
            <Show when={hasDatabase()} fallback={
              <div class="flex flex-col gap-2">
                <label class="text-[10px] text-[#00F0FF] font-black tracking-widest uppercase">Item ID (Comma separated for multiple)</label>
                <input 
                  type="text" 
                  placeholder="e.g. 1, 2, 5"
                  value={manualItemId()}
                  onInput={(e) => setManualItemId(e.target.value)}
              ...
              <div class="flex flex-col gap-2">
                <label class="text-[10px] text-[#00F0FF] font-black tracking-widest uppercase">Select Items ({selectedItemIds().length} selected)</label>
                <input 
                  type="text" 
                  ...
                  <For each={filteredDatabase()}>
                    {([id, name]) => {
                      ...
                      return (
                        <button
                          onClick={() => {
                            if (isSelected()) setSelectedItemIds(prev => prev.filter(i => i !== id));
            ...
            <div class="flex flex-col gap-2">
              ...
              <input 
                type="number" 
                value={newItemQty()}
                onInput={(e) => setNewItemQty(Number(e.target.value))}
          ...
          <div class="flex gap-4 mt-2">
            ...
            <button 
              ...
              disabled={hasDatabase() ? selectedItemIds().length === 0 : !manualItemId().trim()}
        ...
        <div class="flex flex-col gap-6 font-brains">
          <div class="flex flex-col gap-2">
            <p class="text-zinc-400 text-sm tracking-widest uppercase">
              Are you sure you want to completely erase this item from the inventory?
            </p>
            <div class="mt-2 p-3 bg-[#150505] border border-[#FF5252]/30 text-[#FF5252] font-bold tracking-widest text-sm flex items-center justify-between">
              ...
              <span class="text-[10px] font-desc opacity-70">ID: {itemToDelete()?.id}</span>
            </div>
            <p class="text-[10px] text-zinc-500 mt-2 uppercase tracking-widest font-serif">
      ...
      >
        <div class="flex flex-col gap-6 font-brains">
          <div class="flex flex-col gap-2">
            <p class="text-zinc-400 text-sm tracking-widest uppercase">
              Are you sure you want to permanently delete {selectedForBulkDelete().length} item(s)?
            </p>
            <p class="text-[10px] text-[#FF5252] mt-2 uppercase tracking-widest font-sans font-bold">
              Warning: This action cannot be easily undone. All selected items will be removed from your inventory.
            </p>
          </div>
